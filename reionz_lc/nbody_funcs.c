...
void calpow_k(int f_flag, float kmin, float kmax, int Nbin,double* power, double* powerk, double* kmode, long *no)
{
  ...
  int d;
  fftwf_complex *comp_ro;
  float fac1,fac2,fac3, fac, m, scale, m1, d_n;
...
/*-------------------------------------------------------------------------------------------------------*/
int read_sampled(char *fname, int read_flag,long int *seed,int *output_flag,int *in_flag,float **rra,float **vva,float *aa)
{
  FILE *fp1;
  long ii;
  float vaa;
  ...
  *seed=header1.seed;
  ...
  if(read_flag!=1)
      ...
      for(ii=0;ii<MM;ii++)
	{
	  ...
	  fread(&vva[ii][0],sizeof(float),1,fp1);	  
	}
      fread(&dummy,sizeof(dummy),1,fp1);
    }
